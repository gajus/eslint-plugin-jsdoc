...
Unless the `enableFixer` option is set to `false`, an empty JSDoc
block will be added where missing.
...
<Assertions rule="requireJsdoc" />

## Passing assertions

<Assertions rule="requireJsdoc" passing />
