 ...
 * @protected
 */
TypeLexer.prototype.analizeTypeName = function(arg) {
  ...
  if (isModule = str.match(/^module:/)) {
    ...
    if (this.isJsdocGenericType(str)) {
      str = this.analizeJsdocGenericType(str, matched);
    }
      ...
      str = this.analizeGenericType('module:' + matched + str);
    }
    else if (matched) {
 ...
 * @protected
 */
TypeLexer.prototype.analizeJsdocGenericType = function(arg, matched) {
...
TypeLexer.prototype.analizeFunctionType = function(arg) {
  ...
  if (str[0] === '(') {
    ...
    str = this.analizeTypeName(arg.match(/^[Ff]unction/)[0]);
  }
...
TypeLexer.prototype.analizeGenericType = function(arg) {
  ...
  this.analizeTypeName(genericTypeName);
